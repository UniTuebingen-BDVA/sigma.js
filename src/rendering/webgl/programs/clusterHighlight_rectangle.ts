...
export default class ClusterHighlightingRectangleProgram extends AbstractClusterHighlightProgram {
  ...
  process(data: ClusterRectangleData, greyValue: number, offset: number, numPrevPoints?:number): void {
    const array = this.array;
    let i = offset * POINTS * ATTRIBUTES;
    const color = rgbaToFloatColor(greyValue, greyValue, greyValue, 1.0);
    // Triangle 1
    array[i++] = data.xMin;
    array[i++] = data.yMin;
    array[i++] = color;
