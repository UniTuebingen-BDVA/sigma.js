...
// import { NodeFastProgram } from "./rendering/webgl/programs/node.fast.js";
const ClusterHighlightingRectangleProgram = require("./rendering/webgl/programs/clusterHighlight_rectangle.ts");
// console.log(ClusterHighlightingRectangleProgram.default)
/**
 * Constants.
 */
...
var Sigma = /** @class */ (function (_super) {
    ...
    function Sigma(graph, container, settings, additionalData) {
        ...
        _this.edgePrograms = {};
        _this.clusterHighightProgram = undefined;
        ...
        _this.createWebGLContext("clusterHighlights");
        ...
        _this.clusterHighightProgram = new ClusterHighlightingRectangleProgram.default(this.webGLContexts.clusterHighlights, _this); 
    ...
    Sigma.prototype.process = function (keepArrays) {
        ...
        var clusterAreas = additionalData.clusterAreas;
        this.clusterHighightProgram.allocate(clusterAreas.length || 0);
        var nra = 0.025; //nodeRadiusAdjustment
        for (var i = 0, l = clusterAreas.length; i < l; i++) {
            var clusterAreaNorm = {};
            for (var j = 0; j < 2; j++) {
                var norm_xy = {x: clusterAreas[i][j], y: clusterAreas[i][2+j]};
                this.normalizationFunction.applyTo(norm_xy);
                for (let key in norm_xy) {
                    var cur_val = norm_xy[key]; 
                    norm_xy[key] = cur_val <= nra ? -nra: cur_val >= 1-nra ? 1 + nra: cur_val;
                  }
                if (j == 0){
            ...
            this.clusterHighightProgram.process(clusterAreaNorm, i);
        }
    ...
    Sigma.prototype.render = function () {
        ...
        // Darwing Cluster Highlights
        this.clusterHighightProgram.bind();
        this.clusterHighightProgram.bufferData();
        this.clusterHighightProgram.render({
            matrix: this.matrix,
            width: this.width,
            height: this.height,
            ratio: cameraState.ratio,
